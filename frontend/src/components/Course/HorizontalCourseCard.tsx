...
const HorizontalCourseCard = ({ enrollment }: { enrollment: Enrollment }) => {
    ...
    return (
        <CoursePageContext.Provider value={{ course, enrolled: enrollment, setEnrolled }}>
            <article className="flex flex-col md:flex-row lg:w-4/5 lg:h-56 border-2 rounded-lg bg-white overflow-hidden">
                ...
                <div className="flex-1 p-4">
                    <div className="mb-4">
                        ...
                        <p className="text-sm">Taught by {course.instructor.name}</p>
                    </div>

                    <div className="w-full mb-4 md:flex justify-between gap-4 lg:items-center">
                        <div className="flex-1 md:pr-4">{course.shortDescription}</div>
                        {completed ? (
                            <Link
                    ...
                    {completed ? (
                        <div>
                            <CheckCircleOutlined />
                            <span className="pl-2">
                                Completed on {moment(enrollment.updatedAt).format("LL")}
                            </span>
                        </div>
                    ) : (
                        <div className="">
