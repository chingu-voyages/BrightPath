...
const Dashboard = async () => {
  ...
  const { user } = session!;
  // enrolls 
  const enrollments: EnrollmelntOutput[] = await (await fetch(`${process.env.BACKEND_API_URL}/user/${user?.id}/enrollments`)).json();
  const finished = enrollments.filter( enrollement=> enrollement.status === "COMPLETED");
  const currents = enrollments.filter(enrollement => enrollement.status === "ACTIVE");
    ...
    <div className='py-6 flex flex-col items-center gap-2'>
        {/* intro */}
          ...
          <p className='text-lg italic md:w-8/12 border-b border-slate-300 mb-4 w-full'>
              “Success isn't always about greatness. It's about consistency. <br />Consistent hard work leads to success. Greatness will come.” - Dwayne Johnson
          </p>
              <h1 className='text-3xl font-bold mb-4'>Continue Learning</h1>
              <div className='flex flex-col gap-4 '>
                {
                currents.map((enrollement) => (
                        <HorizontalCourseCard key={enrollement.id} {...enrollement} />
                      ))
                }
              </div>
          </section>
